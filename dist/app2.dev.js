...
fs.access('./logs', function (err) {
  // console.log(err);
  if (err != null) {
    fs.mkdirSync('./logs');
    fs.writeFileSync(path.join(process.cwd(), './logs/log.txt'), '', 'utf-8');
    fs.writeFileSync(path.join(process.cwd(), './logs/tel.txt'), '', 'utf-8');
  }
...
app.get('/exe/log', function (req, ress) {
  ...
  var tel = query.tel;
  var telC = '';
  var telText = fs.readFileSync(path.join(process.cwd(), './logs/tel.txt'), 'utf-8').replace(/\n/g, '');
  var info = "";
  ...
  if (telArr.indexOf(tel) == -1 && phone_reg.test(tel)) {
    fs.appendFileSync(path.join(process.cwd(), './logs/tel.txt'), telC);
  ...
  fs.readFile(path.join(process.cwd(), './logs/log.txt'), 'utf-8', function (err, data) {
    if (err) {
      ...
      fs.writeFile(path.join(process.cwd(), './logs/log.txt'), newData, 'utf-8', function (err) {
        if (err) {
          // console.log(err)
          ress.json('2错误');
        } else {
