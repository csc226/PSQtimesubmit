...
$.get('http://127.0.0.1:3500/allDay/userInfo.json', function (res) {
  ...
  function exeTime() {
    ...
    var d = new Date();
    var h = d.getHours(); // console.log('当前时间：',h+':'+m)
    ...
    if (h < 9 || h > 18) {
      ...
      $.ajax({
        type: "get",
        url: "http://127.0.0.1:3000/exe/log?date=" + date + "&count=" + time + "&name=" + arr[time].name + '&tel=' + arr[time].tel,
    ...
    interval = setInterval(exeTime, setTime); // console.timeEnd()
  }

  interval = setInterval(exeTime, setTime); // interval = setInterval(() => {
  //     if (time == arr.length - 1) {
  //         clearInterval(interval)
  //     }
  //     let d = new Date();
